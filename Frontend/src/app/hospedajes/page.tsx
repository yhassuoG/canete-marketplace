...
export default function HospedajesPage() {
  ...
  const hotels = useMemo(
    () => tenants.filter((t) => t.category === "hotel" && t.status === "active"),
    [tenants]
  );

  const districts = useMemo(
    () =>
      Array.from(new Set(hotels.map((t) => t.location).filter(Boolean))).sort((a, b) =>
        a.localeCompare(b)
      ),
    [hotels]
  );
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-4 py-8 md:px-6">
        <section className="rounded-3xl border border-brand-100 bg-hero-nature p-8 shadow-soft md:p-10">
          ...
          <h1 className="mt-2 text-3xl font-bold text-brand-900 md:text-4xl">Hospedajes</h1>
          <p className="mt-2 max-w-2xl text-slate-600">
            Hoteles, hostales, casas de campo y lodges en el Valle de Cañete. Encuentra el lugar
            perfecto para descansar cerca de la naturaleza.
          </p>
        ...
        <section className="space-y-6">
          <h2 className="text-2xl font-bold text-brand-900">
            {loading ? "Cargando..." : `${filtered.length} hospedajes encontrados`}
          </h2>

          {!loading && filtered.length === 0 ? (
            <div className="rounded-3xl border border-slate-200 bg-white p-12 text-center text-slate-500">
              No se encontraron hospedajes con esos filtros.
            </div>
          ) : (
              ...
              {filtered.map((h) => (
                ...
                >
                  <div
                    ...
                    style={h.bannerUrl ? { backgroundImage: `url(${h.bannerUrl})` } : undefined}
                  />
                  <div className="flex flex-1 flex-col gap-2 p-5">
                    <div className="flex items-center justify-between gap-2">
                      <h3 className="text-lg font-bold text-brand-900">{h.name}</h3>
                    ...
                    {h.tagline && (
                      <p className="line-clamp-2 text-sm text-slate-600">{h.tagline}</p>
                    )}
                  </div>
                </Link>
              ))}
